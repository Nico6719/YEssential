...
> 一个简洁、实用的基础插件，适用于基岩版服务器。基于 LegacyScriptEngine (LSE) 的插件，并且可以迁移 TMEssential 的部分数据（如 home 和 warp）以及 [PMenu](https://www.minebbs.com/resources/pmenu.4277/) 的菜单数据（100%兼容！）导入配置文件/数据教程在文末。
## 📌 基本信息
...
|Bstat统计	 |统计BDS信息和在线人数（可在Config内关闭） |	✅|
|Gui修改插件配置	 |网页编辑器。 |	✅|
