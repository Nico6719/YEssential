...
ll.registerPlugin("YEssential", "优化版基础插件", [2, 0, 0], {
    Author: "Nico6719",
    License: "GPL-3.0"
});
...
logger.warn("YEssential 开发版2.0.0-Rc6，作者：Nico6719") 
...
/*cmd.setCallback((cmd,ori,out,res)=>{
    ...
    noticeconf.set("lastNoticeUpdate", Date.now()); // 记录公告最后修改时间
    logger.info("公告已更新，最后修改时间：" + new Date().toLocaleString());
...
cmd.setCallback((cmd, ori, out, res) => {
    ...
    fm.addSwitch("不再显示", noticeconf.get(pl.realName) === 1);
...
/*mc.listen("onServerStarted",()=>{
    ...
    conf.set("lastServerShutdown", Date.now()); // 更新为本次启动时间

    const lastNoticeUpdate = noticeconf.get("lastNoticeUpdate") || 0;
    if (lastNoticeUpdate > lastShutdown) {
        conf.set("showNoticeOnNextJoin", true);
        logger.info("[公告] 检测到更新，将在玩家加入时强制显示");
    }
});
*/
// ======================
...
mc.listen("onServerStarted", () => { 
    ...
    if (lastUpdate > lastShutdown) {
        ...
        logger.info("检测到新公告，玩家下次加入将强制显示");
    }
});
mc.listen("onJoin", (pl) => {
...
mc.listen("onJoin", (pl) => {
    if (conf.get("forceNotice")) {
        conf.set("forceNotice", false);
        noticeconf.set(pl.realName, 0); // 重置玩家设置
        pl.runcmd("notice");
    }
});
// 监听服务器关闭事件（正确名称） ▼
...
function OpenSellMenu(plname) {
    ...
    // 1. 获取玩家背包物品（兼容新版 LiteLoader）
    const playerItems = [];
    for (let slot = 0; slot < 36; slot++) {
        const item = pl.getInventoryItem(slot);
        if (item && item.type !== "minecraft:air") {
            playerItems.push({
    ...
    const validItems = sellItems.filter(shopItem => {
        return playerItems.some(pItem => 
            pItem.type === shopItem.item.toLowerCase() && 
            pItem.meta === (shopItem.meta || 0)
        );
    });

    if (validItems.length === 0) {
        pl.sendText("§c当前没有可出售的物品！");
        return;
    }
    ...
    const fm = mc.newCustomForm()
        .setTitle("§l§a出售物品")
        .addLabel(`您的余额: §e${pl.getScore(conf.get("Scoreboard"))} ${conf.get("CoinName")}`);

    validItems.forEach((item, index) => {
        const total = playerItems
            .filter(pItem => 
                pItem.type === item.item.toLowerCase() && 
                pItem.meta === (item.meta || 0)
            )
            .reduce((sum, pItem) => sum + pItem.count, 0);

        fm.addLabel(`§7${item.name} | 单价: §a${item.price} | 库存: §e${total}`);
        fm.addSlider(`数量_${index}`, 1, total, 1, 1); // 唯一键避免冲突
    });
    ...
    pl.sendForm(fm, (pl, data) => {
        ...
        validItems.forEach((item, index) => {
            const amountKey = `数量_${index}`;
            const amount = data[amountKey];
            
            if (amount > 0) {
                const totalEarn = amount * item.price;
                let remaining = amount;

                // 5. 扣除玩家物品
                for (let slot = 0; slot < 36; slot++) {
                    const invItem = pl.getInventoryItem(slot);
                    if (invItem && 
                        invItem.type.toLowerCase() === item.item.toLowerCase() &&
                        invItem.meta === (item.meta || 0)
                    ) {
                        const take = Math.min(remaining, invItem.count);
                        pl.setInventoryItem(slot, invItem.setCount(invItem.count - take));
                        remaining -= take;
                ...
                // 6. 发放金币
                pl.addScore(conf.get("Scoreboard"), totalEarn);
                pl.sendText(`§a成功出售 ${amount} 个 ${item.name}，获得 §e${totalEarn} ${conf.get("CoinName")}`);
            }
        });
    });
}
